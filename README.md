<h3 align="left">Hi, I'm Gaurav 👋</h3>

I'm an MS Computer Science student at the University of Illinois Chicago (May 2026, GPA 3.89), building agentic AI systems, RAG pipelines, and distributed backends in Python and Go. I'm on OPT and open to full-time **AI Engineer**, **ML Engineer**, and **SDE** roles at startups and mid-sized companies.

📍 Chicago, IL &nbsp;·&nbsp; ✉️ [gaurav.pvt25@gmail.com](mailto:gaurav.pvt25@gmail.com) &nbsp;·&nbsp; 🔗 [LinkedIn](https://www.linkedin.com/in/gauravchintak/) &nbsp;·&nbsp; 🌐 [Portfolio](https://gauravch-code.github.io/Portfolio/)

---

### 🛠 Currently

- Building autonomous multi-agent systems with **CrewAI**, **MCP**, and **LangGraph**
- Contributing to open-source agentic frameworks (**OpenSRE**, exploring **CrewAI** and **Letta** next)
- Researching Transformer-based GAN pipelines on 2TB+ spatialomics data at UIC

---

### 🚀 Featured Projects

**🔧 [Autonomous Agentic SRE Pipeline](https://github.com/gauravch-code/Agentic-SRE-Pipeline)** &nbsp;·&nbsp; `Go` `Python` `MCP` `Kubernetes` `CrewAI` `Gemini`
> Self-healing pipeline that polls Prometheus, diagnoses incidents via an MCP-bridged LLM, and pushes strategic merge patches to resolve OOM errors without human intervention. Go watchdog triggers CrewAI agents that analyze pod logs in a closed remediation loop.

**⚙️ [ToolGen](https://github.com/gauravch-code/toolgen)** &nbsp;·&nbsp; `Python` `OpenAI` `NetworkX` `Pydantic` `Multi-Agent` `LLM-as-Judge`
> Multi-agent synthetic data pipeline that generates multi-turn tool-use conversations grounded in ToolBench schemas. LLM-as-judge scores each conversation on **naturalness, tool correctness, and task completion**, with automatic repair loops on low-quality outputs and cross-conversation diversity steering for training-set variety.

**📚 [AI Research Assistant](https://github.com/gauravch-code/AI-Research-Assistant)** &nbsp;·&nbsp; `Python` `LangChain` `FastAPI` `Pinecone`
> Production RAG API for querying academic documents in natural language. **Sub-200ms p95** under concurrent load with **95% retrieval consistency**. Automated regression testing via GitHub Actions.

**🤖 [AI Documentation Bot](https://github.com/gauravch-code/AI-doc-bot)** &nbsp;·&nbsp; `Python` `LangChain` `Pinecone` `OpenAI`
> Semantic search across Pandas, NumPy, and Scikit-learn docs. MLP classifier ranks results by source reliability, **improving answer relevance by 20%** by prioritizing official API references over community content.

**🎬 [Movie Recommendation Engine](https://github.com/gauravch-code/Movie-Recommendation)** &nbsp;·&nbsp; `Python` `Docker` `FastAPI` `SVD++`
> SVD++ collaborative filtering with automated retraining lifecycle. **0.92 RMSE**, containerized inference behind FastAPI, handling **50+ concurrent requests at sub-200ms p95**.

---

### 🌱 Open Source

**[OpenSRE](https://github.com/Tracer-Cloud/opensre/pull/716)** &nbsp;·&nbsp; ⭐ 1,900+ stars &nbsp;·&nbsp; AI-powered SRE agent framework
> Redesigned the root-cause analysis prompt with a structured multi-step reasoning sequence (hypothesis generation, evidence validation, elimination). Added an alternative-hypothesis output section that **reduced false-positive classifications by 30%**, plus a backward-compatible response parser for mixed-signal incidents.

---

### 🧰 Tech Stack

**Languages** &nbsp; Python &nbsp;·&nbsp; Go &nbsp;·&nbsp; Scala &nbsp;·&nbsp; JavaScript &nbsp;·&nbsp; SQL &nbsp;·&nbsp; C

**AI / ML** &nbsp; PyTorch &nbsp;·&nbsp; TensorFlow &nbsp;·&nbsp; LangChain &nbsp;·&nbsp; CrewAI &nbsp;·&nbsp; MCP &nbsp;·&nbsp; DSPy &nbsp;·&nbsp; PEFT &nbsp;·&nbsp; Transformers &nbsp;·&nbsp; RAG &nbsp;·&nbsp; Pinecone &nbsp;·&nbsp; Scikit-learn

**Backend & Infra** &nbsp; FastAPI &nbsp;·&nbsp; Kubernetes &nbsp;·&nbsp; Docker &nbsp;·&nbsp; Prometheus &nbsp;·&nbsp; Spark &nbsp;·&nbsp; AWS &nbsp;·&nbsp; Azure &nbsp;·&nbsp; GitHub Actions &nbsp;·&nbsp; Linux

**Data & Viz** &nbsp; Pandas &nbsp;·&nbsp; NumPy &nbsp;·&nbsp; OpenCV &nbsp;·&nbsp; Tableau &nbsp;·&nbsp; Streamlit

---

### 📊 GitHub Stats

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=gauravch-code&show_icons=true&hide_border=true&theme=default&count_private=true" height="150" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=gauravch-code&layout=compact&hide_border=true&theme=default&langs_count=8" height="150" />
</p>

---

### 💬 Let's talk

If you're hiring for AI/ML engineering, agentic systems, or distributed backends, I'd love to hear from you. The fastest way to reach me is **[gaurav.pvt25@gmail.com](mailto:gaurav.pvt25@gmail.com)** or a **[LinkedIn](https://www.linkedin.com/in/gauravchintak/)** message.
